...
<body data-bs-theme="dark" class="overflow-x-hidden" id="body">
    ...
    <div class="row align-top g-2 p-3" style="min-height: 100vh;">
        ...
        <div class="col col-md-4 text-center justify-content-center align-items-center pb-5"
            ...
            <?php
            echo "<p>Takip Edilen Hesapların Gönderileri</p>";
            ?>
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                ...
                $sort = $_GET['sort'];
                if ($sort == 'old') {
                    echo "<li><a class='dropdown-item' href='following.php'>En Yeni</a></li>
                            <a class='dropdown-item text-dark bg-light' href='following.php?sort=old'>En Eski</a></li>
                            <a class='dropdown-item' href='following.php?sort=best'>En Beğenilen</a></li>";
                    $sortby = 'id ASC';
                } elseif ($sort == 'best') {
                    echo "<li><a class='dropdown-item' href='following.php'>En Yeni</a></li>
                            <a class='dropdown-item' href='following.php?sort=old'>En Eski</a></li>
                            <a class='dropdown-item text-dark bg-light' href='following.php?sort=best'>En Beğenilen</a></li>";
                    $sortby = 'likes DESC';
                } else {
                    echo "<li><a class='dropdown-item text-dark bg-light' href='following.php'>En Yeni</a></li>
                            <a class='dropdown-item' href='following.php?sort=old'>En Eski</a></li>
                            <a class='dropdown-item' href='following.php?sort=best'>En Beğenilen</a></li>";
                    $sortby = 'id DESC';
                }
            ...
            $following = explode(",", CheckUser($userid, $con)['following']);
            for ($i = 0; $i < count($following); $i++) {
                $following[$i] = "'" . CheckUserID($following[$i], $con)['usertoken'] . "'";
            }
            $following = array_filter($following);
            $following = implode(",", $following);
            $getposts = "SELECT * FROM `content` WHERE publisher IN ($following) ORDER BY $sortby";
