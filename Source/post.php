...
<body data-bs-theme="dark" class="overflow-x-hidden" id="body">
    ...
    <div class="row align-top g-2 p-3" style="min-height: 100vh;">
        ...
        <div class="col col-md-4 text-center justify-content-center align-items-center pb-5"
            ...
            <?php
            $getposts = "SELECT * FROM `content` WHERE id = $id";
            ...
            echo "</div>";
            //echo "<p class='input-group text-secondary text-center'><i class='fa-solid fa-comment-dots me-2'></i><i class='fa-solid fa-arrow-down me-2'></i><button class='flex-fill bg-secondary mt-1' style='height: 2px;' disabled></button></p>";
            ...
            <form action='tool_publish.php' method='post' class='publish'>
                <div class="mb-3">
                    <img class="float-center w-50 rounded mb-2" id="publishimg" src="" onclick="imgRemove()"
                        onerror="this.hide">
                    <textarea class="form-control" placeholder="Yorumun Ne?" name="post" id="post" rows="4"></textarea>
                </div>
                ...
                <input class="d-none" name="related" id="related" value="<?php echo $id ?>">
                <button type="submit" class="btn float-end btn-light">Paylaş</button>
            </form><br><br>
            <p>Gönderinin Yorumları</p>
            <?php

            $getcomments = "SELECT * FROM `content` WHERE related = $id ORDER BY id DESC";
            $comments = mysqli_query($con, $getcomments);
            while ($post = mysqli_fetch_array($comments, MYSQLI_ASSOC)) {
                PostHTML($post, $logined, $userid);
            }
            ?>
